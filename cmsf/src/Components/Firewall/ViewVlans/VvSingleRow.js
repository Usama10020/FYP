        ...
        <tr>
          ...
          <td>
            <button
              ...
              onClick={() => {
                console.log("editVlan");
                setFruleId(test.rid);
                if (userRole === "Admin") {
                  setEditFrule("vlan");
                  navigate("/editVlans");
                } else if (userRole === "Operator") {
                  setEditFrule("vlanOperator");
                  navigate("/editVlans");
                }
              }}
            >
            ...
            <button
              ...
              onClick={async () => {
                if (
                  window.confirm(
                    `Are you sure, you want to delete vlan with Id "${test.rid}"?`
                  )
                ) {
                  if (userRole === "Admin") {
                    ...
                    await AxiosInstance.delete(`/vlans/${test.rid}`)
                      .then((res) => {
                        if (res.data.success === true) {
                          if (
                            ...
                            addLog(`Vlan with Id : "${test.rid} deleted.`);
                    ...
                    AxiosInstance.post("/registerStatuss", {
                      ...
                      description: `Vlan with Id : "${test.rid}" is deleted and waiting for Admin Approval.`,
                      workOnId: test.rid,
                      deviceId: fDeviceId,
                      link: llink,
                    ...
                    addLog(
                      `Vlan with Id : "${test.rid} deleted and waiting for Admin Approval.`
                    );
                    alert(
                      `Vlan with Rule Id : "${test.rid} deleted and waiting for Admin Approval.`
                    );
                  }
                }
              }}
