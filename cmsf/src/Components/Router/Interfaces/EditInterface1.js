...
const EditInterface1 = () => {
  ...
  const registerStatus = (pDesc, crt, edi, dele) => {
    AxiosInstance.post("/registerStatuss", {
      ...
      description: pDesc,
      workOnId: "FastEthernet0/1", //set it later
      deviceId: fDeviceId,
      devicePort: editDevicePort,
      link: "/editInterface1",
    ...
    if (userRole === "Admin") {
            ...
            .then((res) => {
              if (res.data.success === true) {
                if (res.data.message.includes("Apply Changes")) {
                  alert(
                    `FastEthernet0/1 Interface of device "${fDeviceId}" and port "${editDevicePort}" is edited and approved by User Id : "${userId}".`
                  );
                  ...
                  addLog(
                    `FastEthernet0/1 Interface of device "${fDeviceId}" and port "${editDevicePort}" is not created because of an input error.".`
                  );
                }
              }
            })
        ...
        AxiosInstance.post(`/RegisterAndUpdaterIp`, {
          ...
          .then((res) => {
            if (res.data.success === true) {
              if (res.data.message.includes("Apply Changes")) {
                alert(
                  `FastEthernet0/1 Interface of device "${fDeviceId}" and port "${editDevicePort}" is edited.`
                );
                console.log("admin");
                console.log(res.data.message);
                addLog(
      ...
      AxiosInstance.post(`/tempregisterAndUpdaterIp`, {
        ...
        .then((res) => {
          if (res.data.success === true) {
            alert(
              `FastEthernet0/1 Interface of device "${fDeviceId}" and port "${editDevicePort}" is edited and waiting for admin approval.`
            );
            registerStatus(
              `FastEthernet0/1 Interface of device "${fDeviceId}" and port "${editDevicePort}" is edited and waiting for admin approval.`,
              "0",
  ...
  const discardButton = () => {
    if (userRole === "Admin") {
      ...
      addLog(
        `Request for change of FastEthernet0/1 Interface of device "${fDeviceId}" 
